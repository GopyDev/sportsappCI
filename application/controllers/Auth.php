...
class Auth extends CI_Controller {
	...
	function index()
	{
        ...
        function login_json()
        {
                ...
                if ($this->ion_auth->login($this->input->get('username'), $this->input->get('password')))
                {
                        $this->data['data']['login'] = "success";
                        $this->data['data']['userdata'] = $this->session->all_userdata();
                }
                else
                {
                        $this->data['data']['login'] = "failed";
                        $this->data['data']['userdata'] = "";
                }
                $this->_json_out('json/api', $this->data);
        }
        // log the user in
        function signup_json()
        {
            $identity_column = $this->config->item('identity','ion_auth');
            $email    = strtolower($this->input->get('email'));
            $identity = ($identity_column==='email') ? $email : $this->input->get('username');
            ...
            {
                    $this->data['data']['signup'] = "success";
                    $this->data['data']['userdata'] = $this->session->all_userdata();
            }
            else
            {
                    $this->data['data']['signup'] = "failed";
                    $this->data['data']['userdata'] = "";
            }
	...
	function deactivate($id = NULL)
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			// insert csrf check
			$this->data['csrf'] = $this->_get_csrf_nonce();
			$this->data['user1'] = $this->ion_auth->user($id)->row();
